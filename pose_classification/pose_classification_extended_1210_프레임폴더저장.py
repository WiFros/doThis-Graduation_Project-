# -*- coding: utf-8 -*-
"""pose classification extended 1202 각 프레임과 confidence의 쌍의 사본의 사본

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1H5FJwUb89mgEQX-en-3d3aqKe1WcpgvF
...
class PoseClassificationVisualizer(object):
  """Keeps track of claassifcations for every frame and renders them.""" # 각 프레임마다 클래시피케이션 후 렌더하는 
  ...
  def __call__(self,
               ...
               repetitions_count):
    """Renders pose classifcation and counter until given frame.""" #클래시피케이션과 카운터를 각 프레임마다 렌더
    # Extend classification history.
    self._pose_classification_history.append(pose_classification)
    self._pose_classification_filtered_history.append(pose_classification_filtered)

    # Output frame with classification plot and counter. #아웃풋 프레임 = 프레임 이미지 + 그래프(classification plot) + 카운터
...
if not os.path.exists('/content/frame2'):
        os.makedirs('/content/frame2')
...
with tqdm.tqdm(total=video_n_frames, position=0, leave=True) as pbar:
  while True:
    ...
    success, input_frame = video_cap.read() #input_frame이 몸만 난오는 프레임인듯
    if not success:
      break
    ...
    if pose_landmarks is not None:
      ...
      print("\n", pose_classification_filtered.get('pushups_down'), "\n")
      # print("\n", pose_classification_filtered, "\n")
      show_image(input_frame)
      # 여기서 이 둘이 묶어주면 될 듯
      cv2.imwrite('/content/frame2' + "/frame%.2f.jpg" % pose_classification_filtered.get('pushups_down'), input_frame)
    ...
    # Show intermediate frames of the video to track progress. #가끔 중간에 이미지 나오는거
    if frame_idx % 30 == 0:
...
a = os.listdir("/content/frame")
b = os.listdir("/content/frame2")
